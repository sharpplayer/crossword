...
public class CrosswordPanel extends JPanel implements InputListener {
    ...
    private final transient List<CellRenderer> renderers = new ArrayList<>();
    ...
    /** Direct selections. */
    private final transient List<Cell> directSelections = new ArrayList<>();

    /** Indirect selections. */
    private final transient List<List<Cell>> indirectSelections = new ArrayList<>();

    /** Cell selection listener. */
    private final transient CellSelectListener listener;
     ...
     * (non-Javadoc)
     * 
     * @see biz.computerkraft.crossword.gui.input.InputListener#selectCellAt(double, double, boolean)
     */
    @Override
            ...
            for (List<Cell> cells : indirectSelections) {
                if (cells.contains(cell)) {
                    for (Cell selectCell : cells) {
                        select = selectNextCell(newDirectSelection, cell, select, selectCell);
                    }
                }
            }
        }
    ...
    private boolean selectNextCell(List<Cell> newDirectSelection, Cell cell, boolean select, Cell selectCell) {
        if (select) {
            newDirectSelection.add(selectCell);
            return true;
        } else {
            return selectCell.equals(cell);
        }
    }
     ...
     * (non-Javadoc)
     * 
     * @see biz.computerkraft.crossword.gui.input.InputListener#addCellContent(java. lang.String)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see biz.computerkraft.crossword.gui.input.InputListener#deleteCellContent()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see biz.computerkraft.crossword.gui.input.InputListener#backspaceCellContent( )
     */
    @Override
