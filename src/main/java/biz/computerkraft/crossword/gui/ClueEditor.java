...
public class ClueEditor extends JPanel implements ComplexCellEditor {
    ...
    /** Logger. */
    private static final Logger LOGGER = LoggerFactory.getLogger(ClueEditor.class);
    ...
    public ClueEditor(final ClueWriter writer) {
        ...
        clueList.addActionListener(e -> {
            Clue clue = getSelectedItem(false);
            LOGGER.debug("Clue selected {}", clue.getClueId());
        });
    ...
    public final Clue getSelectedItem(final boolean updateClue) {
        Clue clue = null;
        Object text = clueList.getEditor()
                .getItem();
        if (text instanceof String) {
            String textString = text.toString();
            clue = selectClue(clue, textString);
            clue = enableClueButtons(updateClue, clue, textString);
        } else {
            clue = (Clue) text;
    ...
    private Clue enableClueButtons(final boolean updateClue, final Clue newClue, final String textString) {
        Clue clue = newClue;
        if (clue == null) {
            clue = enableListSelectedClueButtons(updateClue, textString);
        } else {
            lastSelected = clue.getClueId();
    ...
    private Clue enableListSelectedClueButtons(final boolean updateClue, final String textString) {
        Clue clue;
     ...
     * (non-Javadoc)
     * 
     * @see biz.computerkraft.crossword.gui.input.ComplexCellEditor#getFirstComponent ()
     */
    @Override
    public final Component getFirstComponent() {
        return clueList.getEditor()
                .getEditorComponent();
    }
     ...
     * (non-Javadoc)
     * 
     * @see biz.computerkraft.crossword.gui.input.ComplexCellEditor#getLastComponent( )
     */
    @Override
