...
public class PropertiesModel extends AbstractTableModel {
	...
	private List<Entry<String, Object>> propertyMap = null;
	...
	public final Object getValueAt(final int rowIndex, final int columnIndex) {
		if (propertyMap == null) {
			...
			if (columnIndex == 1) {
				return propertyMap.get(rowIndex).getValue();
			} else {
				return propertyMap.get(rowIndex).getKey();
			}
		}
	}
	...
	public final void setValueAt(final Object value, final int rowIndex, final int columnIndex) {
		String key = propertyMap.get(rowIndex).getKey();
